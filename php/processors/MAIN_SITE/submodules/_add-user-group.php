...
if (isset($_POST['submit'])) {
    if ($App->can_add_user_permissions) {
        $params = array(
        ...
        if (empty($errors)) {
            ...
            if (is_int($uid)) {
                $App->setSessionMessage("User group added", SUCCESS_STATUS_CODE);
                $App->navigate('/manage/user-groups');
            } else {
                $App->setSessionMessage("An error occured adding the user. Please  try agai n later", 0);
            }
        }
    } else {
