...
if ($action == 'ajax') {
    ...
    if (empty($where)) {
        $where.=" WHERE event='" . $App->event->EVENT_SYNC_COMPLETE . "' OR event='" . $App->event->EVENT_SYNC_UNCOMPLETE . "'";
    } else {
        $where.=" AND event='" . $App->event->EVENT_SYNC_COMPLETE . "' OR event='" . $App->event->EVENT_SYNC_UNCOMPLETE . "'";
    }
    ...
    //$params['event'] = $App->event->EVENT_SYNC_COMPLETE;
    ...
    foreach ($results as $row) {
        $data[] = array(
            ...
            '<a href="/manage/sync-logs/?a=delete&id=' . $row['id_event'] . '" class="btn btn-danger delete-link"  data-toggle="tooltip" data-placement="top" title="Click To Delete Event"><i class="fa fa-trash"></i></a>',
        );
    }
...
if (isset($_POST['delete'])) {
    ...
    header("Location: /manage/sync-logs/");
    exit();
}
...
switch ($action) {
    ...
    case 'delete':
        ...
        $App->LogEevent($App->user->uid, $event, $App->getCurrentDateTime(), "", $id_event);
        header("Location: " . SITE_URL . "/" . ADMIN_FOLDER . '/analytics/sync-logs/');
        exit();
        break;
    default :
        break;
